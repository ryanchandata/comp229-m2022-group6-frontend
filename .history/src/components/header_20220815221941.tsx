...
function toggleLogin()
    ...
    if(isLoggedIn)
    {
      return(
        <><li className='nav-item'>
          <NavLink to={`/account/${ ((getUserId()?.split(":")[4])?.split(",")[0])?.replace(/[""]+/g,'') }`} className='nav-link' aria-current="page"><i className="fa-solid fa-user fa-lg"></i> { ((getUserId()?.split(":")[5])?.split(",")[0])?.replace(/["}"]+/g,'') } </NavLink>
  ...
  function toggleSurveyList()
  {
    ...
    <div className="container-fluid">
      <NavLink to={"/"} className="navbar-brand"><img src={logo} alt="Logo" height={80} width={140} background-color="#ffffff"/><div className="coname"> SurWe - Survey</div></NavLink>
