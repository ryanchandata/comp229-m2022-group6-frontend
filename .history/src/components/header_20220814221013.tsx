...
function Header()
{
  ...
  const [ userString, setUserId ] = useState('');
  const { id } = useParams();
...
function getUserId(id: any)
    {
        ...
        .then((response: any) =>{
            setUserId(response.data.users.userString);
        })
        .catch((e: Error)=>{
            console.log(e);
        });
    ...
    if(isLoggedIn)
    {
      return(
        <><li className='nav-item'>
          <NavLink to={`/account/${ userString.split(",")[0] }`} className='nav-link' aria-current="page"><i className="fa-solid fa-user fa-lg"></i> Account </NavLink>
