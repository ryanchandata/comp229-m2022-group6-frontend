...
{    
    ...
    function saveSurvey(e: any)
    {
        ...
        const data: ISurveyData = {
            _id: ID,
            username: JSON.stringify(((getUserName()?.split(":")[5])?.split(",")[0])?.replace(/["}"]+/g,'')),
            name: name,
        ...
        <div className="container">
            ...
            <form onSubmit={saveSurvey} className="form" method="post">
                <div className="form-group">
                    Created by: <input type="text" value={ ((getUserName()?.split(":")[5])?.split(",")[0])?.replace(/["}"]+/g,'') } disabled />{ ((getUserName()?.split(":")[5])?.split(",")[0])?.replace(/["}"]+/g,'') }<br /><br />
